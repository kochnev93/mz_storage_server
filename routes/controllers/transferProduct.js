    ...
    pool
      ...
      .then((result) => {
        ...
        if (!availabilityProduct) {
          ...
          const getValue = `SELECT id_nomenclature, id_receipt, id_category FROM mz_products WHERE id_warehouse='${old_warehouse}' AND id_product='${id_product}'`;

          pool
            .execute(getValue)
            .then((result) => {
              console.log('DATA', result[0][0]);
              const { id_nomenclature, id_receipt, id_category } = result[0][0];
              const transferProduct = `
                INSERT INTO mz_products 
                (id_product, id_nomenclature, id_receipt, id_category, id_warehouse, count, date_create, id_author)
                VALUES ('${id_product}', '${id_nomenclature}', '${id_receipt}', '${id_category}', '${new_warehouse}', '${transfer_count}', ( SELECT NOW() ), '${id_author}' )`;
